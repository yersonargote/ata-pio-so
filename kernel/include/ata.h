...
/** @brief Partition is NO ACTIVE */
#define NO_ACTIVE 0x0

/** @brief Partition is LINUX type */
...
/** @brief Canales ATA */
typedef enum { ata_primary = 0, ata_secondary = 1 } ata_channel_t;
...
typedef enum { ata_io_primary = 0x1F0, ata_io_secondary = 0x170 } ata_io_port;
 ...
 */
int ata_read(ata_device *dev, char *addr, unsigned int start,
             unsigned char count);
 ...
 */
int ata_write(ata_device *dev, char *addr, unsigned int start,
              unsigned char count);
